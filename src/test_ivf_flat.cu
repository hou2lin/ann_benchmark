...
int main() {
    ...
    auto index = ivf_flat::build<float, int64_t>(dev_resources, index_params, 
    ...
    /* search */
         ivf_flat::search<float, int64_t>(dev_resources, search_params, index, cuvs::make_const_mdspan(queries.view()),
    ...
    if (!dump_to_file) {
        ...
        const char *filename = "ivf_flat_result.txt";
