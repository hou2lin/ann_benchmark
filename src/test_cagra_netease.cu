...
//dispatcher_raft_cagra<float>(n_samples, n_dim, topk, n_queries, batch_size, intermediate_graph_degree, graph_degree, itopk_size, max_iterations, search_width); 
template<typename T>
void dispatcher_raft_cagra(int64_t n_samples, int64_t n_dim, int64_t topk, int64_t n_queries, int64_t batch_size, int64_t intermediate_graph_degree, int64_t graph_degree, int64_t itopk_size, int64_t max_iterations, int64_t search_width, int if_read_exited_data, int split_bast_set_rows) {
     float mean = 0.1;
     float sigma = 1.0;
     uint64_t seed = 233376;
     size_t freeMem, totalMem; //used to cache freemem and totalmem
    /* initialization */
     raft::resources dev_resources; //cuvs handle
     ...
     if (!if_read_exited_data) {
     {
         ...
         auto d_means = raft::make_device_vector<T>(dev_resources, n_dim); // Create a 1-dim device mdarray. Return raft::device_vector.
         RAFT_CUDA_TRY(cudaMemcpy(d_means.data_handle(), means.data(),
                                  sizeof(T) * means.size(), 
                                  cudaMemcpyDefault)); // H2D copy means.
 
                                 raft::random::normal(dev_resources, rng_state, 
                                 dataset.data_handle(), dataset.size(), mean, sigma); // Generate normal distributed table according to the given set of means and scalar standard deviations.
         ...
         #if _PRINT_ORIGINAL_DATA == 1
         {
             ...
             RAFT_CUDA_TRY(cudaMemcpy(h_dataset.data(), dataset.data_handle(),
                                      ...
                                      cudaMemcpyDefault)); // D2H copy dataset.
             RAFT_CUDA_TRY(cudaDeviceSynchronize());
     ...
     // generate random queries, same as dataset
     {
     ...
     cuvs::neighbors::brute_force::search(dev_resources, bf_srch_params, bf_index, 
                                         raft::make_const_mdspan(queries.view()), result_ids.view(), result_distance.view()); // brute-force search topk.   
     RAFT_CUDA_TRY(cudaDeviceSynchronize());
     ...
     index_params.intermediate_graph_degree = intermediate_graph_degree; //Degree of input graph for pruning.
     index_params.graph_degree = graph_degree; //Degree of output graph.
     ...
     UTIL::Timer ttt(true);
     ttt.update_time();
     auto index = cagra::build(dev_resources, index_params, 
                                     raft::make_const_mdspan(dataset.view())); // cagra build index
     RAFT_CUDA_TRY(cudaDeviceSynchronize());
     ...
     /* search */
     const std::size_t num_batches = (n_queries - 1) / batch_size + 1; // total iters for search.
     ...
     for (int batch_id = 0; batch_id < num_batches; ++ batch_id) {
        ...
        /*
        ivf_flat::search<T, int64_t>(dev_resources, search_params, index, raft::make_const_mdspan(queries.view()),
        ...
        */
        //raft::neighbors::cagra::search(dev_resources, search_params, index, queries.data_handle() + row * n_dim, actual_batch_size, topk, result_ids.data_handle() + row * topk, result_distance.data_handle() + row * topk);
        ...
        cuvs::neighbors::cagra::search(dev_resources, search_params, index, queries_v, neighbors_v, distances_v);
 ...
 int main(int argc, char* argv[]) {
     ...
     if (parser.HasOption("--intermediate_graph_degree")) {
        intermediate_graph_degree = std::stoi(parser.GetOptionValue("--intermediate_graph_degree"));
     }
     ...
     */
     ...
     std::cout << "[LOG_INFO] num is : " << n_samples << ", dim is : " << n_dim << ", topk is : " << topk << ", queries is : " << n_queries  << ", batch_size is : " << batch_size << ", intermediate_graph_degree is : " 
     << intermediate_graph_degree << ", graph_degree is : " <<graph_degree << ", itopk_size is : " << itopk_size << ", max_iterations is : " << max_iterations << ", search_width is : " << search_width << std::endl;
     //TODO: it seems that KNN only support float, other types will get compiling error
     dispatcher_raft_cagra<float>(n_samples, n_dim, topk, n_queries, batch_size, intermediate_graph_degree, graph_degree, itopk_size, max_iterations, search_width, if_read_exited_data, split_bast_set_rows); 
