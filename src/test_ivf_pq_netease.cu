...
template<typename T>
void dispatcher_raft_ivf_pq(int64_t n_samples, int64_t n_dim, int64_t topk, int64_t n_queries, int64_t n_lists, int64_t n_probes, int64_t batch_size, int pq_dim, int if_read_exited_data, int split_bast_set_rows) {
     float mean = 0.1;
     float sigma = 1.0;
    ...
    /* initialization */
     ...
     UTIL::Timer ttt(true);
     ttt.update_time();
     auto index = cuvs::neighbors::ivf_pq::build(dev_resources, index_params, 
     ...
     for (int batch_id = 0; batch_id < num_batches; ++ batch_id) {
        ...
        */
        ...
        raft::neighbors::ivf_pq::search(
             dev_resources, search_params, index, queries.data_handle() + row * n_dim, actual_batch_size, topk, result_ids.data_handle() + row * topk, result_distance.data_handle() + row * topk);
        */
        ...
        cuvs::neighbors::ivf_pq::search(dev_resources, search_params, index, queries_v, neighbors_v, distances_v);
 ...
 int main(int argc, char* argv[]) {
     ...
     if (parser.HasOption("--pq_dim")) {
        pq_dim = std::stoi(parser.GetOptionValue("--pq_dim"));
     }
     ...
     std::cout << "[LOG_INFO] num is : " << n_samples << ", dim is : " << n_dim << ", topk is : " << topk << ", queries is : " << n_queries << ", n_lists is : " << n_lists << ", n_probes is : " << n_probes << ", batch_size is : " << batch_size <<" , pq_dim is : " << pq_dim<< std::endl;
     //TODO: it seems that KNN only support float, other types will get compiling error
     dispatcher_raft_ivf_pq<float>(n_samples, n_dim, topk, n_queries, n_lists, n_probes, batch_size, pq_dim, if_read_exited_data, split_bast_set_rows); 
