...
void dispatcher_raft_vamana(int64_t n_samples, int64_t n_dim, int64_t topk, int64_t n_queries, int64_t batch_size, 
                            ...
                            int64_t search_width, int64_t itopk_size, int64_t max_iterations, int if_read_exited_data, 
                            int split_bast_set_rows, int force_rebuild_index, uint32_t diskann_l_search) {
    float mean = 0.1;
    float sigma = 1.0;
    ...
    // Check if Vamana index already exists
    const std::string vamana_graph_file = "vamana_graph.bin";
    bool index_exists = std::filesystem::exists(vamana_graph_file) && !force_rebuild_index;
    
    if (index_exists) {
        std::cout << std::endl;
        std::cout << "[LOG_INFO] Found existing Vamana index: " << vamana_graph_file << std::endl;
        
        // Show index file information
        std::filesystem::path index_path(vamana_graph_file);
        if (std::filesystem::exists(index_path)) {
            auto file_size = std::filesystem::file_size(index_path);
            std::cout << "[LOG_INFO] Index file size: " << (file_size / (1024*1024)) << " MB" << std::endl;
            std::cout << "[LOG_INFO] Skipping index building to save time" << std::endl;
        }
        
        // Note: We don't actually need to load the index for DiskANN search
        // since DiskANN loads from the file directly
    } else {
        std::cout << std::endl;
        if (force_rebuild_index) {
            std::cout << "[LOG_INFO] Force rebuild requested, building new Vamana index..." << std::endl;
        } else {
            std::cout << "[LOG_INFO] No existing index found, building new Vamana index..." << std::endl;
        }
        ...
        // Serialize Vamana graph to file
        cuvs::neighbors::vamana::serialize(dev_resources, vamana_graph_file, vamana_index);
    }
   ...
   //     << std::get<3>(ret) << ", recall_ratio_ret is : " << recall_ratio_ret << std::endl;
    ...
    // --- DiskANN CPU Search ---
    std::cout << std::endl;
    std::cout << "[LOG_INFO] --- Starting DiskANN CPU Search ---" << std::endl;
    
    // Monitor memory before DiskANN operations
    cudaMemGetInfo(&freeMem, &totalMem);
    printMem(freeMem, totalMem, "DiskANN search之前: ");
    
    try {
        DISKANN::DiskANNMemory<T> diskann_search(metric, n_dim);
    typename DISKANN::DiskANNMemory<T>::SearchParam diskann_params;
    diskann_params.L_search = diskann_l_search;
    diskann_params.num_threads = std::min(omp_get_max_threads(), 1); // Limit to 4 threads for stability
    
    UTIL::Timer diskann_load_timer(true);
    diskann_search.Load(vamana_graph_file, diskann_params.num_threads, diskann_params.L_search);
    float diskann_load_time = diskann_load_timer.elapsed_ms();
    std::cout << "[LOG_INFO] DiskANN index load time: " << (diskann_load_time / 1000.0f) << " s" << std::endl;

    T* h_queries = new T[n_queries * n_dim];
    RAFT_CUDA_TRY(cudaMemcpy(h_queries, queries.data_handle(), sizeof(T) * n_queries * n_dim, cudaMemcpyDeviceToHost));
    
    int64_t* h_diskann_ids = new int64_t[n_queries * topk];
    float* h_diskann_distances = new float[n_queries * topk];
    
    UTIL::Perf diskann_perf(n_queries, 1); // Batch size of 1 for CPU search timing
    
    for (int i = 0; i < n_queries; ++i) {
        diskann_perf.timer.update_time();
        diskann_search.Search(h_queries + i * n_dim, 1, topk, diskann_params, h_diskann_ids + i * topk, h_diskann_distances + i * topk);
        diskann_perf.setTime(diskann_perf.timer.elapsed_ms());
    }
    
    auto diskann_ret = diskann_perf.getPerfResult();
    float diskann_recall = bf_gt.compute_recall_ratio(bf_results_h.data(), h_diskann_ids);

        std::cout << "[LOG_INFO] DiskANN Search Parameters: L_search=" << diskann_params.L_search << ", num_threads=" << diskann_params.num_threads << std::endl;
        std::cout << "[LOG_INFO] DiskANN QPS: " << std::get<0>(diskann_ret) << ", Avg Search Time: " << std::get<2>(diskann_ret) << " ms, Recall: " << diskann_recall << std::endl;

        delete[] h_queries;
        delete[] h_diskann_ids;
        delete[] h_diskann_distances;
        
    } catch (const std::exception& e) {
        std::cout << "[LOG_ERROR] DiskANN search failed: " << e.what() << std::endl;
        std::cout << "[LOG_INFO] Skipping DiskANN search due to error" << std::endl;
   
    } catch (...) {
        std::cout << "[LOG_ERROR] DiskANN search failed with unknown error" << std::endl;
        std::cout << "[LOG_INFO] Skipping DiskANN search due to error" << std::endl;
    }
...
int main(int argc, char* argv[]) {
    ...
    // Optimized Vamana parameters for maximum recall
    int64_t graph_degree = 64;      // Higher degree for better connectivity
    int64_t visited_size = 256;      // 2x graph_degree for thorough construction
    float max_fraction = 0.2;        // Balanced value for quality vs speed
    int64_t vamana_iters = 1;        // Multiple iterations for better graph
    
    // Optimized search parameters for maximum recall
    int64_t search_width = 64;       // Higher search width for better coverage
    int64_t itopk_size = 256;        // Larger candidate pool
    int64_t max_iterations = 200;    // More iterations for thorough search
    
    int split_bast_set_rows = 0;
    int if_read_exited_data = 0;
    int force_rebuild_index = 0;  // Force rebuild index even if exists
    uint32_t diskann_l_search = 500;  // Must be >= topk (128) for DiskANN
    ...
    if (parser.HasOption("--force_rebuild_index")) {
       force_rebuild_index = std::stoi(parser.GetOptionValue("--force_rebuild_index"));
    }
    if (parser.HasOption("--diskann_l_search")) {
       diskann_l_search = std::stoi(parser.GetOptionValue("--diskann_l_search"));
    }
    ...
    dispatcher_raft_vamana<float>(n_samples, n_dim, topk, n_queries, batch_size, graph_degree, visited_size, max_fraction, vamana_iters, search_width, itopk_size, max_iterations, if_read_exited_data, split_bast_set_rows, force_rebuild_index, diskann_l_search); 
